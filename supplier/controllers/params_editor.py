...
class ParamsEditor(http.Controller):
    ...
    def editor_get(self, osv_info, editor_id, ):
        ...
        return request.make_response(simplejson.dumps(
            ...
            } for row in getattr(request.env[osv_info.get('model', 'vop.product.template')].browse(int(editor_id)), osv_info.get('field', 'description_editor_ids'))]
        ))
    ...
    def editor_save(self, osv_info, editor_id, delete_items, **args):
            ...
            if str(index) + '-id' in args:
                ...
                if write_id:
                    ...
                    create_val = {
                        ...
                        'value': self._generate_image_path(osv_info.get('model', 'vop.product.template'), editor_id, item),
                        'index': index,
        ...
        write_model = request.env[osv_info.get('model', 'vop.product.template')]
    ...
    def images_get(self, osv_info, editor_id, **kws):
        return request.make_response(simplejson.dumps(
            [{
                ...
                'type': row.jd_product_id.product_type,
                'component': 'image-input',
            } for row in request.env['jd.image'].search([('jd_product_id', '=', int(editor_id))])]
        ))
    ...
    def images_change_order(self, **kws):
        if request.jsonrequest.get('ids') and request.jsonrequest.get('ids') != ['undefined']:
            try:
                request.env.cr.execute("""UPDATE jd_image ji SET
                                            order_sort=(select order_sort + 100  
                                            from jd_image jin where jin.id=ji.id) 
                                             WHERE id in (%s);""" % ','.join(request.jsonrequest.get('ids')))
                for image in request.jsonrequest.get('change_list'):
                    if not image.get('id'):
                        continue
                    request.env.cr.execute("""UPDATE jd_image ji SET  order_sort=%s,is_primary=%s
                                              WHERE id = %s""" % (image.get('index'),
                                                                  not int(image.get('index')),
                                                                  image.get('id')))
                    if not int(image.get('index')):
                        request.env.cr.execute("""UPDATE jd_product jp SET image_path='%s' WHERE sku = '%s'""" %
                                               (image.get('file_url'), image.get('editor_sku')))
    ...
    def images_delete(self, **kws):
        try:
            request.env.cr.execute("""DELETE FROM jd_image WHERE id=%s""" % request.jsonrequest.get('id'))
            return {'result': 'success'}
        except Exception as E:
            return {'result': 'error', 'message': simplejson.dumps(E)}
    ...
    def images_add(self, **kws):
        try:
            file_url = self._generate_image_path('jd.image', kws.get('product_id'), kws.get('file'))
            request.env['jd.image'].create({'jd_product_id': kws.get('product_id'),
                                            'image_path': file_url, 'sku':  kws.get('editor_sku'),
                                            'order_sort': int(kws.get('index')) - 1,
                                            'is_primary': not (int(kws.get('index')) - 1)})
            if kws.get('index') == '1':
                request.env.cr.execute("""UPDATE jd_product jp SET image_path='%s' WHERE id = %s""" %
                                       (file_url, kws.get('product_id')))
        except Exception as E:
            return request.make_response(simplejson.dumps({'result': 'error', 'message': simplejson.dumps(E)}))
